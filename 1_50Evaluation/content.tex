...
They include a build of GCC with support for vector intrinsics (\todoref{GCC appendix}), a binary distribution of Clang/LLVM-13, a custom build of Clang/LLVM-15, and a custom build of CHERI-Clang (based on LLVM 13) with changes to support RVV.
...
Throughout these chapters I have also presented conclusions to a set of hypotheses (\todoref{hyp table}), which prove the general viability of adding scalable vector models to CHERI processsors.
...
Given the importance of vector processing to modern computing, and thus its importance to CHERI, I hope that this research and the software artifacts of this project (see \todoref{appendix of artifacts}) pave the way for future vector-enabled CHERI processor implementations.
