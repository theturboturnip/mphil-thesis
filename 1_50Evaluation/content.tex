...
There is also a special test for fault-only-first: FoF loads are performed at the edge of mapped memory, and the test verifies that out-of-bounds exceptions are swallowed and \code{vl} is set accordingly.
...
This shows the emulator is correctly executing vector instructions, and that it is possible to correctly \code{memcpy} capabilties in vector registers (although further investigation is required to be confident in the security of this approach).
