...
\todomark{if an address range is calculated then: if capability contains it: SUCCESS else if it was wide or FoF: INDETERMINATE else: FAILURE}
\todomark{if an address range isn't calculated: INDETERMINATE}
...
Because all archetypes may have Failure or Indeterminate outcomes, hardware must provide a fallback slow-path for each archetype which checks/performs each access in turn.
...
To avoid this one could assume \code{vstart = 0} and treat failures as Indeterminate for other cases.
%for the calculation, perform the capability check and treat failures as indeterminate (because the failure could be due to an element before \code{vstart}).
