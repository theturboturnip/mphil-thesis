...
In the simple case, \code{vl} is equal to the total available elements (see \todoref{figure - full vl}).
It can also be fewer (see \todoref{figure - partial vl}), in which case vector instructions will not write to elements in the \enquote{tail} (i.e. elements past \code{vl}).
...
This is usually only set by the hardware whenever it is interrupted mid-instruction (see \todoref{figure - vstart}) so that the instruction can be re-executed later without corrupting completed values.
...
However, the true utility of \code{LMUL} lies in widening/narrowing operations (see \todoref{LMUL widening}).
...
\todoref{full diagram} shows all of the above features used in a single configuration:
\begin{itemize}
