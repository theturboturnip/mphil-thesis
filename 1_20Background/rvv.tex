...
Without segmentation, as in \cref{fig:RVV_mem_unit_noseg}, each consecutive memory address maps to a consecutive element in a single vector register group.
With segmentation, elements are grouped into segments of $n$ fields, where each field is mapped to a different vector register group.

\figinput[width=0.48\textwidth,pos=t]{1_20Background/figures/fig_RVV_mem_unit}

\pagebreak
\subsubsection{Unit and Strided accesses}

\figinput[width=0.7\textwidth,pos=h]{1_20Background/figures/fig_RVV_mem_strided}
...
\figinput[width=0.7\textwidth,pos=h]{1_20Background/figures/fig_RVV_mem_indexed}
\begin{tabular}{ll}
\toprule
