...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item In legacy mode, pointers remain \code{XLEN} long and capabilities are still \code{CLEN} long
        \begin{itemize}
            \item Example string: \code{lp32c64}
            \item Integer width (\code{l}) = XLEN = Pointer width (\code{p}) = 32-bits
            \item Capability width (\code{c}) = CLEN = 64-bits
        \end{itemize}
    \end{itemize}
...
\subsection{Pure-capability vector instructions}
\todomark{This requires a definition of how CHERI-RVV instructions should behave - do they *always* take caps, breaking legacy mode? Do they take caps in cap mode but raw addresses in legacy mode?}
\todomark{If the second one, how to do that in LLVM?}
\todomark{Need to add CHERI-enabled but integer-mode toolchain?}
...
% For example, most instructions use general purpose registers to hold integer arguments, so the RISC-V target defines a GPR set.
