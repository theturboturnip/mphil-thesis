...
\todomark{Programmer could use helper macro btut that still requires source code changes}
...
The current specification for RVV intrinsics uses pointer types for all base addresses\cite{specification-RVV-intrinsics-v1.0}.
...
Any scalar code that can be a) compiled in scalar pure-capability mode\footnote{This ensures all memory accesses use valid capabilities.}, and b) auto-vectorized by a legacy RVV compiler, must have an equivalent pure-capability vectorized form.
This form could be acquired by performing the auto-vectorization, then converting the instructions to pure-capability counterparts.
...
Similar models (e.g. Arm SVE) already have auto-vectorization, so RVV auto-vectorization should be possible.
